...
function CloudWatchLogger(logGroupName) {
  ...
  var uploadQueuedLogs = function(logs) {
    _log(
      logGroupName,
      ">> uploadQueuedLogs triggered with ",
      logs.length,
      " logs"
    );
      ...
      Enumerable.from(logs)
        ...
        .forEach(function(group) {
          ...
          if (!(logStreamName in knownLogStreams)) {
            createLogStream = _createLogStreamIfDoesntExist(
              logGroupName,
              logStreamName
            )
              .then(function() {
                knownLogStreams[logStreamName] = true;
              })
              .catch(function(err) {
                console.error(err);
              });
          ...
          createLogStream.then(function() {
            ...
            _uploadLogs(
              ...
              logStreamName,
              logsToUpload,
              nextSequenceTokens[logStreamName]
            )
  ...
  function _createLogGroupIfDoesntExist(name) {
    return _checkLogGroupExists(name).then(function(logGroupExists) {
  ...
  function _checkLogGroupExists(name) {
    ...
    return deferred.promise.timeout(
      settings.aws.timeout || DEFAULT_TIMEOUT,
      "Could not communicate with AWS CloudWatch in a timely fashion"
    );
  }
  ...
  function _createLogGroup(name) {
    ...
    return deferred.promise.timeout(
      settings.aws.timeout || DEFAULT_TIMEOUT,
      "Could not create log group in a timely fashion"
    );
  }

  /* Log streams functions */
  function _createLogStreamIfDoesntExist(group, name) {
    return _checkLogStreamExists(group, name).then(function(logStreamExists) {
  ...
  function _checkLogStreamExists(group, name) {
    ...
    return deferred.promise.timeout(
      settings.aws.timeout || DEFAULT_TIMEOUT,
      "Could not communicate with AWS in a timely fashion"
    );
  }
  ...
  function _createLogStream(group, name) {
    ...
    cw.createLogStream({ logGroupName: group, logStreamName: name }, function(
      err,
      data
    ) {
      if (err) deferred.reject(err);
    ...
    return deferred.promise.timeout(
      settings.aws.timeout || DEFAULT_TIMEOUT,
      "Could not create log stream in a timely fashion"
    );
  }
